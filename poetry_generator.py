...
def generate_rhyming_lines():
    lines = []
    phraseA = random_word_generator(None, 5)
    while True:
        phraseB = random_word_generator(None, 5)
        if phraseB[4] in get_rhymes(phraseA[4]):
            lines.append(' '.join(phraseA))
            lines.append(' '.join(phraseB))
            return lines
...
def generate_10_syllable_lines():
    ...
    while True:
        phraseA = random_word_generator(None, 5)
        if sum([count_syllables(word) for word in phraseA]) == 10:
            while True:
                phraseB = random_word_generator(None, 5)
                if sum([count_syllables(word) for word in phraseB]) == 10:
                    lines.append(' '.join(phraseA))
                    lines.append(' '.join(phraseB))
                    return lines
...
# will be composed of randomly selected words, will contain 10
...
def generate_metered_line():
    while True:
        phraseA = random_word_generator(None, 5)
        if sum([count_syllables(word) for word in phraseA]) == 10:
            if "0101010101" == ''.join([''.join(get_stresses(word)) for word in phraseA]):
                #print(''.join([''.join(get_stresses(word)) for word in phraseA]))
                return ' '.join(phraseA)
...
def generate_line(num):
    while True:
        phraseA = random_word_generator(None, num // 1.6 + 1)
        if sum([count_syllables(word) for word in phraseA]) == num:
            return ' '.join(phraseA)

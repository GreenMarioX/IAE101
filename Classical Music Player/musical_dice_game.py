...
def construct_waltz():
    ...
    trioPlay = [None] * 16
    for i in range(16):
        minuetList[i] = minuet_table[i][roll_dice(2)]
        trioList[i] = trio_table[i][roll_dice(1)]
        minuetPlay[i] = simpleaudio.WaveObject.from_wave_file(minuet_filename(minuetList[i]))
        trioPlay[i] = simpleaudio.WaveObject.from_wave_file(trio_filename(trioList[i]))
    print('Minuet: ', minuetList)
    print('Trio: ', trioList)
    buffer = b""
    for i in range(16):
        buffer += minuetPlay[i].audio_data
    for i in range(16):
        buffer += trioPlay[i].audio_data
    nc = minuetPlay[1].num_channels
    bps = minuetPlay[1].bytes_per_sample
    sr = minuetPlay[1].sample_rate
